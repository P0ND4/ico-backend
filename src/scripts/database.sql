...
-- Exam evaluation results (1 per user/chapter)
CREATE TABLE trn.exam_results (
  id          UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id     UUID        NOT NULL REFERENCES trn.users(id) ON DELETE CASCADE,
  chapter_id  UUID        NOT NULL REFERENCES trn.chapters(id) ON DELETE CASCADE,
...
-- trn.device_trials
CREATE INDEX idx_trn_device_trials_user_id   ON trn.device_trials (user_id);
...
-- trn.path_generation_jobs
CREATE INDEX idx_trn_path_jobs_path_id     ON trn.path_generation_jobs (path_id);
CREATE INDEX idx_trn_path_jobs_status      ON trn.path_generation_jobs (status);
...
-- trn.exam_results
CREATE INDEX idx_trn_exam_results_user_id    ON trn.exam_results (user_id);
CREATE INDEX idx_trn_exam_results_chapter_id ON trn.exam_results (chapter_id);
...
--  ├── subscription_plans     free / pro / premium quotas and pricing
...
--  ├── users                  user records
--  ├── device_trials          per-device quota counters             [N:1 → users]
--  ├── user_plan_quotas       per-user quota counters (pro/premium) [1:1 → users]
--  ├── user_stats             accumulated metrics                   [1:1 → users]
--  ├── user_auth_providers    linked login methods                  [N:1 → users, cat.auth_providers]
...
--  ├── exam_results           exam evaluation per chapter           [1:1 → users, chapters]
...
--  trn.users                  → con.subscription_plans     (plan_code FK)
...
--
-- =============================================================================
-- UPGRADE (bases existentes)
-- =============================================================================
-- Para migrar una BD ya desplegada, ejecutar:
--   psql "$DATABASE_URL" -f src/scripts/upgrade-prod.sql
--
-- Ese script es idempotente (IF NOT EXISTS) y cubre todos los desajustes
-- detectados entre entidades TypeORM y esquemas creados antes de jun/2026.
--
-- =============================================================================
